...
static void init_rgb332_lut(void) {
    for (int i = 0; i < 256; i++) {
        uint8_t r = (i & 0xE0); r |= (r >> 3) | (r >> 6);
        uint8_t g = (i & 0x1C) << 3; g |= (g >> 3) | (g >> 6);
        uint8_t b = (i & 0x03) << 6; b |= (b >> 2) | (b >> 4) | (b >> 6);
...
CGImageRef VNCCreateCGImageFromBuffer(const void *frameBuffer, int width,
    ...
    if (!frameBuffer || width <= 0 || height <= 0) return NULL;
    ...
    CGImageRef image = CGImageCreate(
        width, height, 8, 32, width * 4, colorSpace, bitmapInfo,
